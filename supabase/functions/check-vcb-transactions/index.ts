...
serve(async (req) => {
  ...
  try {
    ...
    if (vcbData.code !== "00") {
      ...
      return new Response(JSON.stringify({ error: "Failed to fetch bank transactions", details: vcbData }), {
        status: 500,
    ...
    if (depositsError) {
      ...
      return new Response(JSON.stringify({ error: "Failed to fetch pending deposits", details: depositsError }), {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
